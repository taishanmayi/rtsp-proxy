	...
	*  	list else add them to the tail end of the list.
	...
	public String debugDump() {
		...
		while (obj != null) {
			...
			try {
				if (Class
					.forName(PackageNames.CORE_PACKAGE + ".GenericObjectList")
					.isAssignableFrom(obj.getClass())) {
					sprint(
						((GenericObjectList) obj).debugDump(this.indentation));
				} else if (
					Class.forName(
						PackageNames.CORE_PACKAGE
							+ ".GenericObject").isAssignableFrom(
						obj.getClass())) {
...
/*
 * $Log: GenericObjectList.java,v $
 * Revision 1.9  2005/04/16 20:38:42  dmuresan
